...
This POC tool can be customized to perform security audits on projects and libraries. Like setting up a dictionary to find the usual suspects of bad coding practices that may lead to a vulnerability. It is important to keep the tool and rules lightweight, so it can easily be deployed with minimal setup in a pipeline or testing/deployment environment. Customization is highly recommended before use, such as adding logging and notifications.
